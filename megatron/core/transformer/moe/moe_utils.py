...
def _centered_fsq_variance_loss(
    ...
    return _centered_fsq_variance_loss_from_expected_indicator(
        expected_indicator,
        denom,
        num_experts,
        reduce_group,
...
def direct_load_balancing_loss_func(
    ...
    if load_balancing_type == "noisy_centered_fsq":
        ...
        if load_balancing_type == "centered_fsq_and_var":
            reusable_margin = (
                ste_margin if ste_rect_poistion == load_balance_ste_rect_poistion else None
            )
...
def _add_pre_activation_metric_logs(
    ...
    for metric_name, log_name in _PRE_ACTIVATION_METRIC_LOG_NAMES.items():
        ...
        log[f"{log_name}/top1_pre_activation"] = float((top1_sum / top_count).item())
        log[f"{log_name}/top2_pre_activation"] = float((top2_sum / top_count).item())
        log[f"{log_name}/avg_pre_activation"] = float((value_sum / value_count).item())
