...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-balance-update-rate', type=float, default=0.0,
                       help='Amount to add to or subtract from the active MoE balance-control '
                            'knob after each training router-metric update. If the current '
                            'MaxVioGlobal is above moe_balance_target_vio the controlled value is '
                            'increased; otherwise it is decreased. The default 0.0 disables the '
                            'update logic.')
